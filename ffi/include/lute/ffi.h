...
static_assert(
    ((sizeof(lua_Integer) <= sizeof(ffi::scalar_stor_t)) && (alignof(lua_Integer) <= alignof(ffi::scalar_stor_t)) && util::is_int<lua_Integer>::value
    ),
...
static_assert(
    ((sizeof(lua_Number) <= sizeof(ffi::scalar_stor_t)) && (alignof(lua_Number) <= alignof(ffi::scalar_stor_t)) &&
...
{
    return static_cast<cdata*>(luaL_testudata(L, idx, lua::CFFI_CDATA_MT));
}
...
#endif
        static_assert(
            util::is_int<lua_Number>::value ? (sizeof(lua_Number) <= sizeof(long long)) : (sizeof(lua_Number) <= sizeof(long double)),
