...
static void make_cdata_func(lua_State* L, void (*funp)(), util::rc_obj<ast::c_function> func, bool fptr, closure_data* cd)
{
    ...
    auto& fud = newcdata(
        L,
        fptr ? ast::c_type{util::make_rc<ast::c_type>(util::move(funct)), 0, ast::C_BUILTIN_PTR} : util::move(funct),
...
static bool prepare_cif_var(lua_State* L, cdata& fud, std::size_t nargs, std::size_t fargs)
{
    ...
    using U = unsigned int;
    return (
        ffi_prep_cif_var(&fud.as<fdata>().cif, to_libffi_abi(func->callconv()), U(fargs), U(nargs), func->result().libffi_type(), targs) == FFI_OK
    );
}
...
static void* from_lua(lua_State* L, ast::c_type const& tp, void* stor, int index, std::size_t& dsz, int rule)
{
    ...
    case LUA_TSTRING:
        if ((rule == RULE_CAST) ||
            ((tp.type() == ast::C_BUILTIN_PTR) && ((tp.ptr_base().type() == ast::C_BUILTIN_CHAR) || (tp.ptr_base().type() == ast::C_BUILTIN_VOID)) &&
...
static void from_lua_table_record(lua_State* L, ast::c_type const& decl, void* stor, std::size_t rsz, int tidx, int sidx, int ninit)
{
    ...
    bool empty = true;
    sb.iter_fields(
        [L, rsz, val, &decl, &sidx, &filled, &empty, &ninit, tidx, uni](char const* fname, ast::c_type const& fld, std::size_t off)
        {
            empty = false;
            if (tidx && (ninit < 0))
            {
