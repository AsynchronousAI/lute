...
#define KEYWORDS \
    KW(alignof), KW(alignas), KW(auto), KW(const), KW(enum), KW(extern), KW(sizeof), KW(struct), KW(signed), KW(typedef), KW(union), KW(unsigned), \
        KW(volatile), KW(void), \
\
        KW(_Alignas), \
\
...
static bool parse_type_ptr(lex_state& ls, ast::c_type& tp, util::strbuf* fpname, bool needn, bool tdef, bool& tdef_bltin)
{
    ...
    newlevel:
        ...
        if (!nolev)
        {
            ...
            pcvq.back().cpponv = conv;
        }
        else
        {
            pcvq.back().cpponv = ast::C_FUNC_DEFAULT;
        }
    ...
    if (nolev)
    {
        ...
        pcvq[pidx].cpponv = conv;
        if (pcvq[pidx].cpponv == ast::C_FUNC_DEFAULT)
        {
    ...
    for (std::intptr_t ridx = tidx - 1;;)
    {
        ...
        if (ls.t.token == '(')
        {
            ...
            clev.cpponv = conv;
            if (clev.cpponv == ast::C_FUNC_DEFAULT)
            {
                clev.cpponv = prevconv;
            }
        }
        else if (ls.t.token == '[')
        {
...
using signed_size_t = util::conditional_t<
    ...
    signed char,
    util::conditional_t<
        sizeof(std::size_t) == sizeof(short),
        short,
