...
std::ptrdiff_t c_record::field_offset(char const* fname, c_type const*& fld) const
{
    std::ptrdiff_t ret = -1;
    iter_fields(
        [fname, &ret, &fld](char const* ffname, ast::c_type const& ffld, std::size_t off)
        {
...
std::size_t c_record::iter_fields(
    bool (*cb)(char const* fname, ast::c_type const& type, std::size_t off, void* data),
    void* data,
    std::size_t obase,
    bool& end
) const
...
void decl_store::commit()
{
    ...
    p_dmap.for_each(
        [this](char const* key, c_object* value)
        {
