...
struct cdata_meta
{
    ...
    static int index(lua_State* L)
    {
        ...
        if (index_common<false>(
                L,
                [L](auto& decl, void* val)
                {
    ...
    static bool cmp_base(lua_State* L, ast::c_expr_binop op, ffi::cdata const* cd1, ffi::cdata const* cd2)
    {
        ...
        if (((cd1->decl.type() != ast::C_BUILTIN_PTR) || (cd2->decl.type() != ast::C_BUILTIN_PTR)) ||
...
#undef FIELD_CHECK
    ...
    {
        auto& cd = ffi::checkcdata(L, 1);
        ffi::newcdata(L, ast::c_type{util::make_rc<ast::c_type>(util::move(cd.decl.unref())), 0, ast::C_BUILTIN_PTR}, sizeof(void*)).as<void*>() =
            cd.address_of();
        return 1;
    }
