 ...
 * when compiling as DLL, the 'luteopen_cffi' symbol must be dllexport, while
 * for a static lib it must be unmarked.
 *
 * On Unix-like platforms, all symbols are hidden by default if supported by
 * the compiler, with 'luteopen_cffi' being the sole visible symbol, this is
...
extern "C" CFFI_LUA_EXPORT int luteopen_cffi(lua_State* L)
{
    ffi_module_open(L);
    return 1;
}
